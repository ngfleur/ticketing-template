...
export function HomeScreen({ evenements }: HomeScreenProps) {
  return (
      ...
      <div className="mx-auto relative">
        ...
        <div className="bg-zinc-900 text-site pt-16 sm:p-20">
          <div className="px-8 sm:px-0 max-w-4xl mx-auto" data-testid="tickets.container">
            <h1 className="uppercase text-4xl sm:text-7xl text-center sm:text-left">SHOWS</h1>
            <div className="py-10">
                ...
                >
                  <div className="flex flex-1 sm:items-center gap-4 sm:gap-4 md:gap-8 flex-col sm:flex-row sm:py-3">
                      ...
                      <div className="flex flex-col min-w-fit sm:flex-row overflow-hidden sm:gap-4 md:gap-8 relative">
                        ...
                        <div className="flex gap-4 items-center absolute bottom-2 left-2 sm:bottom-auto sm:left-auto sm:relative">
                          <span className="text-4xl">
                            {new Date(evenement.dateDebut).getDate().toString().padStart(2, '0')}
                          </span>
                          <div className="flex flex-col text-xs">
                            <span className="text-white sm:text-gray-600">
                              {new Date(evenement.dateDebut).toLocaleDateString('fr-FR', { weekday: 'short' })}
                            </span>
                            <span>
                              {new Date(evenement.dateDebut).toLocaleDateString('fr-FR', { month: 'short' })}
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="grow flex flex-col hover:text-purple-500">
                      <button className="text-left w-full h-full group/button text-2xl text-left">
                        {evenement.titre}
                        <svg
                          fill="none"
                      ...
                      <div className="text-sm text-site transition-all ease-in pointer-events-none opacity-0 h-0">
                        <p>{formatDate(evenement.dateDebut)}</p>
                        <p>{evenement.lieu}</p>
